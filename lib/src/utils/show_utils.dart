...
class ShowUtils {
  ...
  static void tooltipSingleEvent(
      BuildContext context, CalendarEvent event, Color color) {
    final text =
        'From: ${event.start.hour}:${event.start.minute.toString().padLeft(2, '0')}\n'
  ...
  static void showSingleEventDetails(
      BuildContext context, CalendarEvent event, Color color) {
  ...
  static Widget _detailEvent(CalendarEvent event,
      ...
      Column(
        ...
        children: [
          ...
          Text('End: ${event.end}',
              style: const TextStyle(color: Colors.white)),
          Visibility(
              visible: paddingBottom, child: const SizedBox(height: 10.0))
        ],
      );

  static void showEventsDetails(
      BuildContext context, List<CalendarEvent> events, Color color) {
    showModalBottomSheet(
      backgroundColor: color,
      context: context,
      builder: (_) => Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: events
                .map((event) => _detailEvent(event,
                    paddingBottom: event == events.last ? false : true))
                .toList(),
          )),
    );
  }
